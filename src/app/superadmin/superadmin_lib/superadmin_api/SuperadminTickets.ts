...
export const ticketsApi = {
  listTickets() {
    let SuperadminTickets = db.getAll<Ticket>('spg_tickets' as unknown);
    if (SuperadminTickets.length === 0) {
      ...
      dummy.forEach(d => db.insert('spg_tickets' as unknown, d as unknown));
  ...
  createTicketOnBehalf(data: { ownerId: string, title: string, description: string, priority: string }) {
    ...
    db.insert('spg_tickets' as unknown, ticket as unknown);
    return ticket;
  },
  
  updateTicketStatus(id: string, status: string) {
    db.update<Ticket>('spg_tickets' as unknown, id, { status } as unknown);
  }
};
