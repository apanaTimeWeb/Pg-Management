...
import { SuperAdminOwnerRequestsTableProps } from '../SuperAdminOwnerRequests_types/SuperAdminOwnerRequests.types';
    ...
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-lg,12px)] shadow-sm">
      <div className="overflow-x-auto max-h-[60vh] custom-scrollbar">
        <table className="w-full text-sm text-left">
          <thead className="bg-[var(--bg-card)] border-b border-[var(--border)] text-[var(--text-secondary)] uppercase text-[12px] font-semibold sticky top-0 z-10 shadow-sm shadow-black/5">
            <tr>
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {loading ? (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-[var(--text-secondary)] motion-safe:animate-pulse">
                  Loading requests...
                </td>
              </tr>
            ) : requests.length === 0 ? (
              ...
              requests.map((r) => (
                <tr 
                  key={r.id} 
                  className="h-12 even:bg-black/5 dark:even:bg-white/[0.02] hover:bg-[var(--primary-subtle)] transition-colors group cursor-default"
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="text-[14px] font-medium text-[var(--text-primary)]">{r.pgCount} PGs</div>
                    <div className="text-[12px] text-[var(--text-secondary)]">{r.bedCount} Beds expected</div>
                  </td>
                    ...
                    {r.status === 'Pending' && (
                      <div className="flex justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                        <button 
                          ...
                          className="p-1.5 text-[var(--success)] hover:bg-[var(--success-bg)] rounded-[var(--radius-md,8px)] transition-colors" 
                        ...
                        <button 
                          ...
                          className="p-1.5 text-[var(--warning)] hover:bg-[var(--warning-bg)] rounded-[var(--radius-md,8px)] transition-colors" 
                          title="Put on Hold"
                        ...
                        <button 
                          ...
                          className="p-1.5 text-[var(--danger)] hover:bg-[var(--danger-bg)] rounded-[var(--radius-md,8px)] transition-colors" 
                          title="Reject"
