      ...
      <div className="bg-[var(--bg-card)] w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleResolveSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            {errors.repairCost && <p className="text-xs text-[var(--danger)] mt-1">{errors.repairCost.message}</p>}
          ...
          <div className="pt-2 flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 py-3 bg-[var(--bg-input)] text-[var(--text-primary)] rounded-xl font-bold hover:bg-[var(--border)] transition-colors">Cancel</button>
            <button type="submit" className="flex-1 py-3 bg-[var(--success)] text-white rounded-xl font-bold shadow-lg shadow-[var(--success)]/20 hover:bg-[var(--success-hover,green)] transition-colors">Confirm & Resolve</button>
          </div>
        </form>
      </div>
    </div>
