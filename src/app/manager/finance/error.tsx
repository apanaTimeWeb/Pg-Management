...
export default function ErrorBoundary({ ManagerError, reset }: { ManagerError: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] p-6 text-center animate-in fade-in duration-300">
      <div className="w-16 h-16 bg-[var(--danger-bg,rgba(239,68,68,0.1))] text-[var(--danger,#EF4444)] rounded-full flex items-center justify-center mb-4">
      ...
      <button 
        onClick={() => reset()}
        className="px-6 py-2 bg-[var(--primary)] text-white rounded font-bold hover:bg-[var(--primary-hover)] transition-colors"
      >
        Try Again
      </button>
    </div>
